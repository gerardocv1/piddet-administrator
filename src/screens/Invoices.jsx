...
export function Invoices() {
  ...
  const columns = [
    ...
    { key: 'customer_name', header: 'Cliente', render: (r) => r.customer_name?.trim() || <span className={s.faint}>—</span> },
    { key: 'origin_code', header: 'Origen', render: (r) => originLabel(r.origin_code) },
    { key: 'service_type', header: 'Servicio', render: (r) => serviceTypeLabel(r.service_type) },
    {
      key: 'status', header: 'Estado',
      render: (r) => { const st = orderStatusOf(r.status); return <Badge variant={st.variant} dot>{st.label}</Badge>; },
    },
    {
      key: 'status_payment', header: 'Pago',
      render: (r) => { const st = paymentStatusOf(r.status_payment); return <Badge variant={st.variant}>{st.label}</Badge>; },
    },
    { key: 'total_formatted', header: 'Total', align: 'right', render: (r) => <span className={s.priceCell}>{r.total_formatted}</span> },
  ];
