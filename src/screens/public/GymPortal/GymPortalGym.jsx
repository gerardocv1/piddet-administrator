...
import { ChatIcon, ChevronDownIcon, ClockIcon, NavigationIcon, PhoneCallIcon, PinIcon } from './icons.jsx';
...
function StoreCard({ store, gymName, whatsapp, single }) {
  ...
  return (
    <section className={s.card} aria-label={single ? 'Tu gimnasio' : store.name}>
      <div className={s.head}>
        <div className={s.titles}>
          <span className={s.eyebrow}>{single ? 'Tu gimnasio' : gymName}</span>
          <h2 className={s.name}>{single ? (gymName || store.name) : store.name}</h2>
        </div>
        {hasSchedule && (
