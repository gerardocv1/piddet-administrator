...
// PRUEBA a un número escrito a mano —texto libre o cualquier mensaje del negocio con datos de
// ejemplo—, que entra al historial como una más. Es de la compañía activa: el backend saca el
// `company_id` de la ruta, no del cliente.
//
...
export const sentNotificationsService = {
  ...
  // Qué se puede probar: `custom` (texto libre) y cada mensaje del negocio (recordatorio de
  // llegada, resumen diario, avisos del gimnasio) con su `message` de ejemplo, renderizado por el
  // backend con la misma clase que usa el envío real.
  getTestNotificationKinds: () => http.get(`${base()}/test/kinds`),
  // SMS de prueba al número indicado. Con `kind = custom` va `message` tal cual; con otro tipo, el
  // backend manda el ejemplo de ese mensaje. Queda en el historial con motivo MANUAL_TEST y se
  // cobra como cualquier otro; si la pasarela lo rechaza en el acto, el backend responde 502 con
  // el motivo.
  sendTestNotification: ({ to, kind, message }) =>
    http.post(`${base()}/test`, { to, kind, message }),
