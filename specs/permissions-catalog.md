...
| `api-module-general-options` | `/general-options` | Opciones generales: grupos de opciones de la compañía (p. ej. «Servicios»: despachar, cubiertos) que aplican a **varios productos** a la vez. Cada grupo tiene sus opciones y una lista de productos asignados, que se elige en un modal con búsqueda por nombre y filtro por categoría (`PUT /companies/{company}/general-option-groups/{id}/items` reemplaza la lista completa). El menú del POS los expone dentro de cada producto asignado, detrás de sus grupos propios. En la API cada acción tiene su permiso `general-option-list/create/update/delete`. |
...
| `api-module-rentable-units` + funcionalidad `functionality_reservations` | `/rentable-units` | Configuración de unidades rentables (cabañas, habitaciones, lugares), sus espacios, inclusiones y fotos, incluido **duplicar** una unidad completa bajo un nombre nuevo (copia también las fotos). Los servicios adicionales salen de los ítems tipo servicio del catálogo de productos. |
| `reservation-managers-config` + funcionalidad `functionality_reservations` | `/reservations/managers` | **Encargados de reservas**: los empleados de la compañía que reciben cada noche (20:00) el resumen por SMS de las reservas que llegan mañana —unidad, titular, personas, hora, si está sin confirmar— con la **decoración** y los **servicios adicionales** que hay que preparar. Se eligen de la lista de empleados (el backend rechaza a un cliente); la pantalla avisa si no hay ninguno o si alguno no tiene celular, porque ese SMS no sale. Es configuración de la compañía (super-admin y `company-admin`), separada del permiso de operar reservas. |
...
| `api-module-gym` + funcionalidad `functionality_gym` | `/` (widgets del Dashboard) | Los widgets de gimnasio del inicio: **Afiliados** (`GET /gym/dashboard/members-summary`: cuántos están activos, en gracia —con el saldo por cobrar— y cancelados; cada contador abre Afiliados filtrado por membresía), **Vencimientos** (`GET /gym/dashboard/expiring`: suscripciones en gracia o que vencen en los próximos días, con saldo) y **Cumpleaños del mes** (`GET /gym/dashboard/birthdays`: afiliados activos que cumplen años este mes, con saludo por WhatsApp). |
| `gym-subscriptions-create` | (acción) | Dar de alta o renovar la suscripción de un afiliado a un plan (con pago inicial opcional). Si el afiliado tiene una suscripción vigente, la nueva se encadena automáticamente desde el día siguiente a su vencimiento. También *Generar período siguiente* en el detalle de la suscripción (`POST /companies/{company}/gym/subscriptions/{id}/process`): fuerza el ciclo diario del backend para esa suscripción cuando el cron no corrió —crea el período siguiente pendiente de pago o, si el vigente agotó su gracia sin abonos, aplica el corte—. Y *Mover inicio* en el período vigente (`PUT …/subscriptions/{id}/periods/{periodId}/start-date`): cambia la fecha de inicio del último período vivo y el backend recalcula vencimiento y gracia con la duración del período; nunca se solapa con el anterior y los pagos no cambian. |
| `gym-subscriptions-cancel` | (acción) | Cancelar una suscripción vigente (irreversible, motivo obligatorio). También *Cancelar período* en el menú ⋮ del período vigente (`PUT …/subscriptions/{id}/periods/{periodId}/cancel`): cancela el último período vivo y con él la suscripción; si el período tiene pagos activos, el diálogo pregunta si se anulan también con sus facturas (casilla visible solo con `gym-payments-annul`). |
...
| Permiso | Rutas | Qué habilita |
|---|---|---|
| `api-module-shifts` | `/shifts`, `/shifts/open`, `/shifts/:shiftId`, `/shifts/:shiftId/close` | Visibilidad de todos los turnos de la compañía y gestión de los de cajero y de compras. Un turno es **global** (por compañía), **de cajero** o **de compras** (uno o **varios** usuarios asignados: caja compartida, `assigned_user_ids` al abrir y `assigned_users` en la respuesta; un usuario puede estar en varios turnos abiertos, pero no se admite un segundo turno del mismo tipo con exactamente los mismos usuarios: 409 con sus nombres); se abre con una base de dinero y el backend le asocia automáticamente ventas y gastos como movimientos con monto y método denormalizados. El **de compras** (`PURCHASE`) no recibe ventas: su balance es base + **adiciones** − gastos; las adiciones se registran desde el balance del detalle (`api.addShiftAddition` → `POST /shifts/{id}/additions` con `{amount, payment_method, notes?}`, con el mismo doble permiso del módulo) y su cierre solo registra la diferencia, sin factura ni gasto de respaldo. El detalle muestra balance en vivo; el cierre es paso a paso (contar dinero → balance = base + ventas − gastos → confirmar) y registra sobrante/faltante como ajuste. Con el turno ABIERTO, desde el menú «⋯» junto a *Cerrar turno*: corregir la base (`api.updateShiftBase` → `PUT /shifts/{id}/base`) o cancelar el turno con motivo obligatorio (`api.cancelShift` → `POST /shifts/{id}/cancel`, irreversible: queda `CANCELLED`, no recibe más movimientos ni bloquea nuevas aperturas; sus movimientos se conservan como historial). Ambas acciones exigen `api-module-shifts` (sin variante `own`) y, para el turno global, además `shift-global-admin`. También gatea la acción rápida de turnos del Dashboard. |
| `api-module-shifts-own` | `/shifts` y subrutas | Acceso de cajero: abre, cierra y ve **solo los turnos en los que está asignado** (el backend filtra por la lista de asignados, `shift_users`), y registra adiciones en su turno de compras. Al abrir, el turno es suyo (de cajero o de compras): no elige a otros. |
| `shift-global-admin` | (acción) | **Único** permiso que abre y cierra el turno GLOBAL: gatea la opción en `/shifts/open` y el botón de cierre. El backend lo exige y además hace visible el turno global a quien solo tenga el acceso de cajero. El global no se cierra con turnos de cajero o de compras abiertos (409). |
...
| `api-module-notifications` | `/notifications` | Historial de lo que la compañía ha **enviado** (SMS y push): a quién salió cada mensaje, con qué texto, por qué motivo y en qué estado quedó, con filtros por fecha, estado, canal, motivo y búsqueda libre (destinatario o texto). A diferencia de la bitácora del scheduler, esto **sí** es de la compañía activa —el backend saca el `company_id` de la ruta—, por eso lo llevan el `super-admin` y el `company-admin`: quien paga los SMS los audita. No se edita ni se borra nada; lo único que admite es **reenviar** una notificación (crea otra fila; una fallida sale directo, una enviada o pendiente exige *Forzar reenvío* porque se paga otra vez) y **enviar un SMS de prueba** a un número escrito a mano, que entra al historial como una más y también se cobra. No confundir con la campana del usuario, que es lo que **una persona** no ha leído. |
| `api-module-scheduled-tasks` | `/scheduled-tasks` | Permiso de **plataforma** (solo `super-admin`): bitácora de las ejecuciones del scheduler —qué tarea corrió, cuándo, cuánto tardó, qué hizo y el error si falló—, filtrable por tarea, resultado y rango de fechas. Lo que muestra NO es de la compañía activa: los comandos recorren todas (endpoints bajo `/companies/{company}/scheduled-tasks…`, donde `{company}` solo sirve para el chequeo del permiso, mismo patrón que `item-category-master`). Desde aquí también se **lanza** una tarea a mano (o en modo prueba) cuando el cron no corrió. |
...
| `company-master` | `/admin/companies`, `/admin/companies/:companyId` | Permiso de **plataforma** (solo `super-admin`): listado de TODAS las compañías, alta de compañías nuevas, activar/inactivar cualquiera y administrar qué usuarios pertenecen a cada una — todo sin cambiar de compañía activa. Endpoints bajo `/companies/{company}/master/companies…`, donde `{company}` solo sirve para el chequeo del permiso (mismo patrón que `item-category-master`). El alta pide nombre, `username` (dirección web pública, única) y su administrador, que se **busca por teléfono** y recibe el rol `company-admin`. |
...
| `role-assign` | (acción) | Abre el selector de permisos del rol (exige además `permission-list`). El selector agrupa por módulo en desplegables colapsados —se abren solos al buscar— y muestra la clave del permiso con su descripción debajo. **Guardar REEMPLAZA la lista completa del rol**, así que el modal avisa cuántos permisos se retiran antes de aplicar. El backend protege los roles del sistema: `super-admin` no se puede reducir (por definición tiene todos los permisos; se recupera con el seeder `RestoreSuperAdminPermissions`) y `client`/`employee` exigen además `admin-general`; el panel deshabilita el botón en esos casos. |
...
| `functionality_gym` | Habilita el módulo de Gimnasio (afiliados, suscripciones, planes y medidas). |
