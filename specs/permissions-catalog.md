...
| `api-module-gym` + funcionalidad `functionality_gym` | `/` (widgets del Dashboard) | Los dos widgets de gimnasio del inicio: **Vencimientos** (`GET /gym/dashboard/expiring`: suscripciones en gracia o que vencen en los próximos días, con saldo) y **Cumpleaños del mes** (`GET /gym/dashboard/birthdays`: afiliados activos que cumplen años este mes, con saludo por WhatsApp). |
